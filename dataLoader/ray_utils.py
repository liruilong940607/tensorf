...
def depth2dist(z_vals, cos_angle):
    ...
    dists = torch.cat(
        [dists, torch.Tensor([1e10]).to(device).expand(dists[..., :1].shape)],
        -1,
    )  # [N_rays, N_samples]
...
def ray_marcher(rays, N_samples=64, lindisp=False, perturb=0, bbox_3D=None):
    ...
    z_steps = torch.linspace(
        0, 1, N_samples, device=rays.device
    )  # (N_samples)
    ...
    xyz_coarse_sampled = rays_o.unsqueeze(1) + rays_d.unsqueeze(
        1
    ) * z_vals.unsqueeze(
        2
    )  # (N_rays, N_samples, 3)
...
def ndc_bbox(all_rays):
    ...
    far_min = torch.min(
        (all_rays[..., :3] + all_rays[..., 3:6]).view(-1, 3), dim=0
    )[0]
    far_max = torch.max(
        (all_rays[..., :3] + all_rays[..., 3:6]).view(-1, 3), dim=0
    )[0]
