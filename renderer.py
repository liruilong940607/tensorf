    ...
    for idx, samples in tqdm(
        ...
        if len(test_dataset.all_rgbs):
            ...
            if compute_extra_metrics:
                ...
                l_a = rgb_lpips(
                    gt_rgb.numpy(), rgb_map.numpy(), "alex", tensorf.device
                )
                l_v = rgb_lpips(
                    gt_rgb.numpy(), rgb_map.numpy(), "vgg", tensorf.device
                )
                ssims.append(ssim)
                l_alex.append(l_a)
                l_vgg.append(l_v)
...
def evaluation_path(
    ...
    imageio.mimwrite(
        f"{savePath}/{prtx}video.mp4", np.stack(rgb_maps), fps=30, quality=8
    )
    imageio.mimwrite(
        f"{savePath}/{prtx}depthvideo.mp4",
        np.stack(depth_maps),
