...
class TensorBase(torch.nn.Module):
    def __init__(
        ...
        self.init_render_func(
            shadingMode, pos_pe, view_pe, fea_pe, featureC, device
        )

    def init_render_func(
        ...
        if shadingMode == "MLP_PE":
            ...
            self.renderModule = MLPRender(self.app_dim, view_pe, featureC).to(
                device
            )
        elif shadingMode == "SH":
    ...
    def getDenseAlpha(self, gridSize=None):
        ...
        for i in range(gridSize[0]):
            alpha[i] = self.compute_alpha(
                dense_xyz[i].view(-1, 3), self.stepSize
    ...
    def updateAlphaMask(self, gridSize=(200, 200, 200)):
        ...
        ks = 3
        alpha = F.max_pool3d(
            alpha, kernel_size=ks, padding=ks // 2, stride=1
        ).view(gridSize[::-1])
        ...
        print(
            f"bbox: {xyz_min, xyz_max} alpha rest %%%f"
