...
class TensorVM(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        self.plane_coef = torch.nn.Parameter(
            0.1
            * torch.randn(
                (3, self.app_n_comp + self.density_n_comp, res, res),
                device=device,
            )
        )
        self.line_coef = torch.nn.Parameter(
            0.1
            * torch.randn(
                (3, self.app_n_comp + self.density_n_comp, res, 1),
                device=device,
            )
        )
    ...
    def vectorDiffs(self, vector_comps):
        ...
        for idx in range(len(vector_comps)):
            ...
            non_diagonal = dotp.view(-1)[1:].view(n_comp - 1, n_comp + 1)[
                ..., :-1
            ]
...
class TensorVMSplit(TensorBase):
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        ...
        return torch.nn.ParameterList(plane_coef).to(
            device
    ...
    def vector_comp_diffs(self):
        return self.vectorDiffs(self.density_line) + self.vectorDiffs(
            self.app_line
        )
